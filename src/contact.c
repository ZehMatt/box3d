...
{
	bool wasScalar = ( contact->flags & b3_simMeshContact ) != 0 || contact->subCount > 1;
	...
	if ( extraIndex >= contact->extraCapacity )
	{
		...
		contact->extraCapacity = newCapacity;
	}
	...
	*sub = ( b3SubContact ){ 0 };
	...
	// A second sub forces the scalar solver path; migrate off the wide list
	if ( wasScalar == false && contact->colorIndex != B3_NULL_INDEX )
	{
		int colorIndex = contact->colorIndex;
		int localIndex = contact->localIndex;
		b3RemoveContactFromGraph( world, contact->edges[0].bodyId, contact->edges[1].bodyId, colorIndex, localIndex, false );
...
static void b3DestroySubContactData( b3World* world, b3Contact* contact, b3SubContact* sub )
{
...
void b3RemoveSubContact( b3World* world, b3Contact* contact, int subIndex, bool wakeBodies )
{
	...
	b3DestroySubContactData( world, contact, sub );
...
void b3DestroyContact( b3World* world, b3Contact* contact, bool wakeBodies )
{
	for ( int i = 0; i < contact->subCount; ++i )
	{
		b3DestroySubContactData( world, contact, b3GetSubContact( contact, i ) );
	}
	...
	if ( contact->colorIndex != B3_NULL_INDEX )
	{
		...
		B3_ASSERT( contact->setIndex == b3_awakeSet );
		bool meshContact = ( contact->flags & b3_simMeshContact ) != 0 || contact->subCount > 1;
