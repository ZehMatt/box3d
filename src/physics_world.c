...
static void b3CollideTask( int startIndex, int endIndex, int workerIndex, void* context )
{
	...
	for ( int i = startIndex; i < endIndex; ++i )
	{
		...
		// Update the scalar contact spec (mesh or multi-sub contacts live on the scalar list)
		if ( touching == true && wasTouching == true && ( ( contact->flags & b3_simMeshContact ) || contact->subCount > 1 ) )
		{
	...
	for ( uint32_t k = 0; k < bitSet->blockCount; ++k )
	{
		...
		while ( bits != 0 )
		{
			...
			else if ( flags & b3_simStoppedTouching )
			{
				...
				bool isMeshContact = ( contact->flags & b3_simMeshContact ) != 0 || contact->subCount > 1;
	...
	for ( int contactIndex = 0; contactIndex < contactCount; ++contactIndex )
	{
		...
		if ( setId == b3_awakeSet )
		{
			if ( touching )
			{
				...
				if ( ( contact->flags & b3_simMeshContact ) != 0 || contact->subCount > 1 ||
